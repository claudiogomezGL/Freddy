...
public struct JSONParser {
    ...
    public mutating func parse() throws -> JSON {
        ...
        guard loc == input.count else {
            throw JSONError.endOfStreamGarbage(offset: loc)
        }
        return value
    }
    ...
    private mutating func parseValue() throws -> JSON {
        guard depth <= ParserMaximumDepth else {
            throw JSONError.exceededNestingLimit(offset: loc)
        }
        
        guard input.count > 0 else {
            throw JSONError.endOfStreamUnexpected
        }
        ...
        throw JSONError.valueInvalid(offset: loc, character: UnicodeScalar(input[loc]))
    }
    ...
    private mutating func guardAgainstUnsupportedEncodings() throws {
        ...
        guard JSONEncodingDetector.supportedEncodings.contains(encodingPrefixInformation.encoding) else {
            throw JSONError.invalidUnicodeStreamEncoding(detectedEncoding: encodingPrefixInformation.encoding)
        }
    ...
    private mutating func decodeNull() throws -> JSON {
        guard input.index(loc, offsetBy: 3, limitedBy: input.count) != input.count else {
            throw JSONError.literalNilMisspelled(offset: loc)
        }
    ...
    private mutating func decodeTrue() throws -> JSON {
        ...
        if     input[loc+1] != Literal.r
            ...
            throw JSONError.literalTrueMisspelled(offset: loc)
        }
    ...
    private mutating func decodeFalse() throws -> JSON {
        ...
        if     input[loc+1] != Literal.a
            ...
            throw JSONError.literalFalseMisspelled(offset: loc)
        }
    ...
    private mutating func decodeString() throws -> JSON {
        ...
        while loc < input.count {
            ...
            case Literal.BACKSLASH:
                ...
                default:
                    throw JSONError.controlCharacterUnrecognized(offset: loc)
                }
                loc = (loc + 1)
    ...
    private mutating func readUnicodeEscape(_ start: Int) throws {
        guard let codeUnit = readCodeUnit() else {
            throw JSONError.unicodeEscapeInvalid(offset: start)
        }
        ...
        while loc < input.count {
            ...
            if !items.isEmpty {
                guard loc < input.count && input[loc] == Literal.COMMA else {
                    throw JSONError.collectionMissingSeparator(offset: start)
                }
                loc = (loc + 1)
            }
    ...
    private mutating func decodeObject() throws -> JSON {
        ...
        while loc < input.count {
            ...
            guard loc < input.count && input[loc] == Literal.DOUBLE_QUOTE else {
                throw JSONError.dictionaryMissingKey(offset: start)
            }
...
private struct NumberParser {
    ...
    mutating func parseNegative() throws {
        ...
        loc = (loc + 1)
        guard loc < input.count else {
            throw JSONParser.JSONError.endOfStreamUnexpected
        }
        ...
        default:
            throw JSONParser.JSONError.numberSymbolMissingDigits(offset: start)
        }
    }
    ...
    mutating func parseDecimal() throws {
        ...
        default:
            throw JSONParser.JSONError.numberMissingFractionalDigits(offset: start)
        }
    }
